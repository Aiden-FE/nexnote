...
type Theme = 'editorial' | 'workbench' | 'graph' | 'quiet';
type Lang = 'en' | 'zh';
const themes: Theme[] = ['editorial', 'workbench', 'graph', 'quiet'];
const copy = {
  en: {
    nav: ['Features', 'Scenes', 'FAQ'], gallery: 'Prototype gallery', language: '中文',
    ...
    features: 'One knowledge base. Four ways to see it.', scenes: 'Product scenes', placeholder: 'Internal prototype panel · not a real screenshot',
    ...
    open: 'Open prototype', back: 'Back to gallery', directions: { editorial: 'Editorial Knowledge', workbench: 'Local-first Workbench', graph: 'Graph Intelligence', quiet: 'Quiet Precision' },
    descriptions: { editorial: 'A considered reading room for ideas, references, and long-form thinking.', workbench: 'A dependable instrument for pages, commits, and deliberate AI actions.', graph: 'See the relationships behind your knowledge, then follow the next useful edge.', quiet: 'A focused, precise surface that lets the work—not the interface—lead.' },
  ...
  zh: {
    nav: ['功能', '实景', 'FAQ'], gallery: '原型画廊', language: 'English',
    eyebrow: '本地优先的知识，安静地彼此连接', title: '你的知识，应该始终属于你。',
    sub: 'NexNote 将块编辑、双链、Git 历史与显式原生 AI 层，放进一个从容的桌面应用。',
    features: '一个知识库，四种观看方式。', scenes: '产品实景', placeholder: '内部原型面板 · 非真实截图',
    download: '下载最新版本', github: '在 GitHub 查看', faq: '常见问题，一次回答。', footer: '为用链接思考的人打造的本地优先知识库。',
    open: '打开原型', back: '返回画廊', directions: { editorial: 'Editorial Knowledge', workbench: 'Local-first Workbench', graph: 'Graph Intelligence', quiet: 'Quiet Precision' },
    descriptions: { editorial: '为想法、参考与长篇思考准备的阅读室。', workbench: '管理页面、提交与审慎 AI 操作的可靠工具。', graph: '看见知识背后的关系，沿着下一条有用的边继续探索。', quiet: '专注、精准的界面，让工作而不是界面成为主角。' },
...
function pathFor(lang: Lang, theme?: Theme) { return theme ? `${lang === 'zh' ? '/zh' : ''}/proto/${theme}` : lang === 'zh' ? '/zh' : '/'; }
function currentRoute(): { lang: Lang; theme?: Theme } { const parts = location.pathname.split('/').filter(Boolean); const lang: Lang = parts[0] === 'zh' ? 'zh' : 'en'; const candidate = (lang === 'zh' ? parts[2] : parts[1]) as Theme | undefined; return { lang, theme: candidate && themes.includes(candidate) ? candidate : undefined }; }
...
function Chrome({ lang, text, children, gallery = false }: { lang: Lang; text: Text; children: React.ReactNode; gallery?: boolean }) { return <><header className="site-header"><Link href={pathFor(lang)} className="brand"><Mark /><span>NexNote</span></Link><nav>{text.nav.map((item, i) => <Link key={item} href={gallery ? `#${['features','scenes','faq'][i]}` : `#${['features','scenes','faq'][i]}`}>{item}</Link>)}</nav><div className="header-actions"><Link href={lang === 'zh' ? '/' : '/zh'} className="language">{text.language}</Link><Link href="https://github.com/Aiden-FE/nexnote" className="github">GitHub</Link></div></header>{children}</>; }
