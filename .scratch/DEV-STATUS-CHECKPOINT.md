...
- **当前状态（2026-09-22）**：验收反馈本轮（DEV-075～DEV-081）已建票；**DEV-076（同步冲突 UX & 转圈收尾，P0）已完成**并分两轮合入 master（首轮 `a7d3c45`、覆盖补齐 `4cec30b`）。当前 master `4cec30b`。
- **DEV-076 首轮（候选 `b3dab03`，merge `a7d3c45`）**：状态机抽到 `packages/renderer/src/features/git/state-machine.ts`；GitStatusItem 冲突徽标可点 / hover 实质文案 / spinner 终态收尾 / dismiss 重置；main `configureAutoSync` 把 `syncProgressListener` 注入 `sync().onProgress`（修掉自动同步静默）；ipc 索引层注册 `services.git.onSyncProgress` 广播。
- **DEV-076 二轮（候选 `d998788`，merge `4cec30b`）**：新增 `git-status-item.test.tsx` 真实组件集成测试，覆盖冲突可点/hover/doctor 弹出/降级/error 收尾/done 清理/dismiss 重置共 6 例；main 测试改为等价行为验证（configureAutoSync 注入 listener）。**本轮 NOT_RUN 已清零**（GUI 冲突路径由 happy-dom 组件集成测试覆盖；完整 Electron packaged smoke 仍是发布前独立门禁）。
- **DEV-076 验收门禁**：`CI=true pnpm -r typecheck` PASS；Vitest **168 files passed / 1 skipped、1554 passed / 2 skipped**；lint 0 errors；build PASS；release-config **31/31**；changed-format 与 diff-check PASS。
