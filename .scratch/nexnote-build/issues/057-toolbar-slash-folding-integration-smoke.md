...
- 浏览器真实路径：`typeWithKeyboard` 逐字符 keydown + Chromium `execCommand('insertText')`，TipTap 与 CodeMirror 各独立键入 `/h2`，不用内部 slash hook 或注入菜单状态；共享目录单测新增模式、英文/中文别名、能力和上下文过滤。核对既有 `slash-menu-dom.test.ts` / `source-slash-menu.test.tsx` 均挂载真实编辑器并经输入事件触发；两种菜单的稳定选择器在测试中约束 `role`、accessible name、历史行为。
...
- packaged smoke **RUN, FAIL**：证据 `.scratch/nexnote-build/smoke/DEV-057/results.json`，243/261 checks PASS（18 FAIL）；CodeMirror `/h2`、嵌套折叠、目录 reveal、全部展开、重开与字节对比均 PASS。TipTap `/h2` smoke 的重开 tab 注册时序失败；源码 toolbar 选择器、DEV-047 既有源码格式化/插入链和 DEV-048 预览滚动共出现失败（详见 results.json），不得声称全部通过。原先 180s 超时无法写回新结果，延至 600s 后得到明确失败清单。跨平台 Windows/Linux packaged smoke **NOT_RUN**（当前 macOS arm64 环境）；由对应 runner 打包、按其 Electron ABI staging、运行 `NEXNOTE_APP_PATH=<binary> pnpm smoke:ci` 并检查 results.json。
