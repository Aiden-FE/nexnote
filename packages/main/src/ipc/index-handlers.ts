...
export function registerIndexHandlers(registrar: IpcRegistrar): void {
  registrar.register('index:status', (_payload, services): Result<IndexStatus> =>
    ok(services.index.status),
  );
  registrar.register('index:rebuild', (_payload, services) => ok(services.index.rebuild()));
  registrar.register('index:backlinks', ({ pagePath }, services) =>
    ok(services.index.backlinks(pagePath)),
  );
  registrar.register('index:search', ({ query, limit }, services) =>
    ok(services.index.search(query, limit)),
  );
  registrar.register('index:jumpTo', ({ query, limit }, services) =>
    ok(services.index.jumpTo(query, limit)),
  );
  ...
  registrar.register('index:pageSummary', ({ path }, services) =>
    ok(services.index.pageSummary(path)),
  );
  registrar.register('index:pageSummaries', (_payload, services) =>
    ok(services.index.pageSummaries()),
  );
  registrar.register('index:graph', (_payload, services): Result<GraphSnapshot> =>
    ok(services.index.graph()),
  );
  registrar.register('index:confidence', ({ pageId }, services): Result<ConfidenceResult | null> =>
    ok(services.index.getConfidence(pageId)),
  );
