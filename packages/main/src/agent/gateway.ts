...
export const AGENT_SCENARIO_PROFILES: Record<AgentScenario, { system: string; tools: string[] }> = {
  ...
  // 临时翻译只读：无任何工具（含写工具）可执行；reasoning 由请求参数强制关闭。
  translation: {
...
export class AgentGateway {
  ...
  async run(scenario: AgentScenario, request: AgentRunRequest): Promise<{ runId: string }> {
    ...
    // 临时翻译（DEV-041）：拒绝缺字段请求，并强制关闭 reasoning——不信任渲染层 params。
    ...
    // translation 场景：prompt 由主进程组装；writing 场景：白名单 actionId + 选区/上下文。
    const messages = request.translation
