...
describe('DocxService', () => {
  it('导入→编辑副本→导出，原件字节 sha256 保持不变', async () => {
    ...
    const imported = await service.importDocx({ externalPath: source }, '');
    ...
    expect(copy).toMatchObject({ path: 'outside (副本).md', created: true });
