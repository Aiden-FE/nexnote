...
import { ArrowLeft, ArrowRight, Bot, Check, Cloud, CloudCog, Loader2, RefreshCw, Server, X } from 'lucide-react';
...
const PROVIDER_PRESETS: Array<{
  ...
  { id: 'openai', title: 'OpenAI 官方', desc: 'api.openai.com，需要官方 API Key', kind: 'openai-compatible', baseUrl: 'https://api.openai.com/v1', icon: Cloud },
  { id: 'azure', title: 'Azure OpenAI', desc: '企业级部署，模型名 = deployment 名', kind: 'azure-openai', baseUrl: 'https://YOUR-RESOURCE.openai.azure.com', icon: CloudCog },
  { id: 'ollama', title: '本地 Ollama', desc: '本地推理，无需密钥', kind: 'openai-compatible', baseUrl: 'http://127.0.0.1:11434/v1', icon: Server },
  { id: 'compatible', title: '其他 OpenAI 兼容', desc: '中转站 / 自建网关 / vLLM 等', kind: 'openai-compatible', baseUrl: '', icon: Bot },
];
...
function WizardBody({ editProfileId, onClose }: { editProfileId: string | null; onClose: () => void }) {
  ...
  const runTest = useCallback(
    async (silent = false) => {
      ...
      setTestError(null);
      try {
        const target = editing && !apiKey
          ? { profileId: editing.id }
          : { candidate: { kind, baseUrl, apiKey: apiKey || undefined } };
        const result = await invoke('ai:testConnection', target);
        setTestResult(result);
        setModels(result.models);
        if (!silent && result.models.length > 0 && !result.models.includes(model)) {
          setModel(result.models.find((m) => !m.toLowerCase().includes('embed')) ?? result.models[0]!);
        }
        return result;
      } catch (e) {
    ...
    [apiKey, baseUrl, editing, kind, model],
  );
  ...
  const finish = async () => {
    ...
    try {
      ...
      if (!editing) {
        ...
        const chatModel = saved.state.profiles.find((p) => p.id === saved.id)?.defaultModel ?? model;
      ...
      <div className="flex max-h-full w-full max-w-lg flex-col overflow-hidden rounded-xl border bg-card text-card-foreground shadow-2xl">
        ...
        <div className="min-h-0 flex-1 overflow-auto px-5 py-4 text-sm">
            ...
            <div data-testid="ai-wizard-connection" className="space-y-3.5">
              {!isEdit && (
                <p className="text-xs text-muted-foreground">
                  ...
                  <button type="button" className="ml-2 underline" onClick={() => setStep('provider')}>
                    重选
                  </button>
                </p>
              )}
              <Field label="Profile 名称">
                <Input data-testid="ai-wizard-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="如：OpenAI 官方 / 本地 Ollama" />
              </Field>
              <Field label="Base URL" hint={kind === 'azure-openai' ? '形如 https://资源名.openai.azure.com' : '通常以 /v1 结尾'}>
                <Input data-testid="ai-wizard-baseurl" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} placeholder="https://api.example.com/v1" spellCheck={false} />
              </Field>
              <Field label="API Key" hint={editing ? '留空 = 保留已保存密钥' : kind === 'openai-compatible' && presetId === 'ollama' ? '本地服务可留空' : '仅存本机系统钥匙串，不会上传'}>
                <Input data-testid="ai-wizard-apikey" type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder={editing?.hasApiKey ? '••••••（已保存）' : 'sk-…'} autoComplete="off" />
              </Field>
            </div>
          )}
          ...
          {step === 'test' && (
            <div data-testid="ai-wizard-test" className="space-y-3">
              <p className="text-muted-foreground">向供应商发送最小探测请求（列模型 + 一次对话 + 一次向量）。</p>
              <Button data-testid="ai-wizard-test-btn" variant="outline" size="sm" disabled={testing} onClick={() => void runTest()}>
                {testing ? <Loader2 className="size-3.5 animate-spin" /> : <RefreshCw className="size-3.5" />}
                {testing ? '测试中…' : testResult ? '重新测试' : '测试连通性'}
              </Button>

              {testError && (
                <div data-testid="ai-wizard-test-error" className="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive">
                  {testError}
              ...
              {testResult && (
                <div data-testid="ai-wizard-test-result" className="space-y-2 rounded-md border p-3 text-xs">
                  <div className="flex items-center gap-2 font-medium">
                    {testResult.reachable ? (
                      <><Check className="size-4 text-green-600" /> 连接成功 · {testResult.latencyMs}ms</>
                    ) : (
          ...
          {step === 'model' && (
            <div data-testid="ai-wizard-model" className="space-y-3">
              <Field label="默认对话模型" hint={models.length > 0 ? `来自供应商模型列表（共 ${models.length} 个）` : '供应商未返回列表，可手动输入'}>
                <Input
                  ...
                  {models.slice(0, 12).map((m) => (
                    <button
                      ...
                      className={cn(
                        ...
                        model === m ? 'border-primary bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-accent/50',
                      )}
                    >
                      {m}
                    </button>
              ...
              {saveError && (
                <div className="rounded-md border border-destructive/40 bg-destructive/10 p-2 text-xs text-destructive">{saveError}</div>
              )}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between gap-2 border-t px-5 py-3">
          ...
          <div className="flex gap-2">
            {step === 'connection' && (
              <Button data-testid="ai-wizard-next" size="sm" disabled={!canNext} onClick={() => setStep('test')}>
                下一步
                <ArrowRight className="size-3.5" />
              </Button>
            )}
            {step === 'test' && (
              <Button data-testid="ai-wizard-to-model" size="sm" disabled={!testResult?.reachable} onClick={() => setStep('model')}>
                下一步
                <ArrowRight className="size-3.5" />
              </Button>
            )}
            {step === 'model' && (
              <Button data-testid="ai-wizard-finish" size="sm" disabled={!canNext || saving} onClick={() => void finish()}>
