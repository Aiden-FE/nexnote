...
import { activePageRef, activeSelectionText, addPageContext, addSelectionContext } from './chat-context-bridge';
  ...
  return (
    <span className="relative">
      ...
      >
        ...
        <span className="shrink-0 text-[10px] text-muted-foreground">~{estimateTokens(chip.text)}</span>
        <span
          role="button"
          tabIndex={0}
          aria-label="移除上下文"
      ...
      {open && (
        <span
          data-testid="context-chip-detail"
          className="absolute bottom-full left-0 z-20 mb-1 max-h-40 w-64 overflow-auto rounded-md border bg-popover p-2 text-[11px] leading-relaxed text-popover-foreground shadow-md"
        >
...
function AddMenu() {
  ...
  const pages = useMemo(
    () =>
      entries
        ...
        .filter((e) => (filter ? titleFromPath(e.name).toLowerCase().includes(filter.toLowerCase()) : true))
        .slice(0, 30),
    [entries, filter, ref],
  );
  ...
  return (
    <span className="relative">
      ...
      {open && (
        <span className="absolute bottom-full right-0 z-30 mb-1 w-64 rounded-md border bg-popover p-1.5 text-[11px] text-popover-foreground shadow-md">
          <button
            type="button"
            disabled={!sel}
