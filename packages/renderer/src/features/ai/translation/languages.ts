...
export function mapInterfaceLanguageToTranslationTarget(
  ...
  if (tag.toLowerCase().startsWith('zh')) return '简体中文';
  if (tag.toLowerCase().startsWith('en')) return 'English';
  if (tag.toLowerCase().startsWith('ja')) return '日本語';
  if (tag.toLowerCase().startsWith('ko')) return '한국어';
  if (tag.toLowerCase().startsWith('fr')) return 'Français';
  if (tag.toLowerCase().startsWith('de')) return 'Deutsch';
  if (tag.toLowerCase().startsWith('es')) return 'Español';
  if (tag.toLowerCase().startsWith('ru')) return 'Русский';
  return undefined;
}
