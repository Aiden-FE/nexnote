...
const PLACEHOLDER_GLOBAL_RE = new RegExp(`${ANCHOR_OPEN}(${BLOCK_ID_RE_SOURCE})${ANCHOR_CLOSE}`, 'g');
...
export function replaceAnchorsWithPlaceholders(markdown: string): string {
  ...
  const out = lines.map((line) => {
    ...
    return line.replace(ANCHOR_SUFFIX_RE, (_all, id: string) =>
      ` ${ANCHOR_OPEN}${id.slice(1)}${ANCHOR_CLOSE}`,
    );
  });
  return out.join('\n');
}
...
function isText(node: JSONContent | undefined): node is JSONContent & { type: 'text'; text: string } {
...
function extractOnlyPlaceholderParagraph(
  ...
  const m = new RegExp(`^${ANCHOR_OPEN}(${BLOCK_ID_RE_SOURCE})${ANCHOR_CLOSE}[ \\t]*$`).exec(only.text);
  if (!m) return null;
...
function withoutBlockId(attrs: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
...
function stripIds(node: JSONContent): JSONContent {
  ...
  const isInjectable = node.type === 'paragraph' || node.type === 'heading' || node.type === 'listItem';
