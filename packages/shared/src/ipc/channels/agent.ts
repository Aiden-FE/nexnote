...
export const AGENT_CHANNELS = [
  'agent:run:chat', 'agent:run:writing', 'agent:run:debug',
  'agent:cancel', 'agent:approval:respond',
] as const;
...
export interface AgentChannelMap {
  ...
  'agent:approval:respond': { request: AgentApprovalResponse; response: Result<{ accepted: boolean }> };
}
export function scenarioChannel(scenario: AgentScenario): AgentChannel { return `agent:run:${scenario}` as AgentChannel; }
