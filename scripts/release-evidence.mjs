...
export function parseImmutableEvidenceUrl(raw) {
  ...
  if (url.protocol !== 'https:' || url.username || url.password || url.port || url.search || url.hash) {
    throw new Error('QA evidence URL must be plain HTTPS without credentials, port, query, or fragment');
  }
  // raw.githubusercontent.com avoids mutable GitHub HTML endpoints and redirects.
  const parts = url.pathname.split('/').filter(Boolean);
  if (url.hostname !== 'raw.githubusercontent.com' || parts.length < 4 || parts[0] !== 'Aiden-FE' || parts[1] !== 'nexnote') {
    throw new Error('QA evidence URL must be an immutable raw blob from Aiden-FE/nexnote');
  }
  if (!/^[0-9a-f]{40}$/i.test(parts[2])) throw new Error('QA evidence URL must pin a full immutable 40-character commit SHA');
...
export async function fetchEvidence(rawUrl) {
  ...
  try {
    ...
    if (length && (!Number.isSafeInteger(length) || length > MAX_EVIDENCE_BYTES)) throw new Error('QA evidence exceeds size limit');
...
export function validateAttestation(attestation, want, evidenceCommit) {
  if (!attestation || typeof attestation !== 'object' || Array.isArray(attestation)) throw new Error('QA evidence must be a JSON object');
  for (const key of ['repository', 'tag', 'commit', 'channel']) {
    if (attestation[key] !== want[key]) throw new Error(`fetched QA evidence ${key} does not bind this release`);
  }
  ...
  if (!/^[0-9a-f]{40}$/.test(evidenceCommit)) throw new Error('invalid immutable evidence source commit');
  if (attestation.allRequiredChecksPassed !== true || want.allRequiredChecksPassed !== true) throw new Error('all required QA checks must be explicitly attested');
}

export async function create(output) {
  const want = expected();
  if (want.repository !== 'Aiden-FE/nexnote') throw new Error('release evidence only supports canonical Aiden-FE/nexnote');
  if (!/^[0-9a-f]{64}$/.test(want.evidenceSha256)) throw new Error('QA evidence SHA-256 must be 64 lowercase hex');
  if (!['stable', 'beta', 'alpha'].includes(want.channel)) throw new Error('invalid release channel');
  const { body, evidenceCommit } = await fetchEvidence(want.evidenceUrl);
  const actualSha256 = createHash('sha256').update(body).digest('hex');
  if (actualSha256 !== want.evidenceSha256) throw new Error('fetched QA evidence SHA-256 does not match supplied digest');
  let attestation;
  try { attestation = JSON.parse(body.toString('utf8')); } catch { throw new Error('fetched QA evidence must be UTF-8 JSON'); }
  validateAttestation(attestation, want, evidenceCommit);
  const manifest = { schemaVersion: 2, ...want, evidenceCommit, fetchedSha256: actualSha256, fetchedBytes: body.byteLength, attestation };
...
export function validate(data) {
  ...
  for (const key of ['repository', 'runId', 'tag', 'commit', 'channel', 'evidenceUrl', 'evidenceSha256', 'allRequiredChecksPassed']) {
  ...
  if (data.evidenceCommit !== evidenceCommit || data.fetchedSha256 !== data.evidenceSha256 || data.fetchedBytes > MAX_EVIDENCE_BYTES) {
