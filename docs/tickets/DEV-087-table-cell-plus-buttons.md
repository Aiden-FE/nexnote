...
- [ ] 鼠标焦点在表格某行任意单元格时，该行左下角出现加号图标；点击后该行下方新增一行，光标合理落位
- [ ] 鼠标焦点在表格某列任意单元格时，该列右上角出现加号图标；点击后该列右侧新增一列，光标合理落位
- [ ] 鼠标移出表格区域（hover 离开）时加号图标淡出或消失
- [ ] 多个相邻 cell 选区（CellSelection）下加号仍可见且可点击；不闪烁 / 不重复出现
- [ ] 列宽拖拽、块级 drag-handle、工具栏「表格」菜单、slash 命令面板命令入口 全部不回归
- [ ] 新增单测：plugin 装饰计算（按 mock 选区得到正确 row/col index）、点击回调触发正确命令
