...
- [x] 光标在表格内时，工具栏在原插入位出现一个「表格」菜单（而非 4 个独立按钮）
- [x] 菜单内 4 项可见、可点击，行为与原先 4 个按钮一致
- [x] 光标离开表格后该菜单自动消失（与原 inTable 行为一致）
- [x] 工具栏溢出逻辑不受影响；窄窗口下仍能正确折叠
- [x] 现有 slash 命令、快捷键、命令面板触发表格操作的行为不变
- [x] `pnpm typecheck` / `pnpm lint` / 相关 Vitest 全绿；`slash-table-actions` 测试仍过
