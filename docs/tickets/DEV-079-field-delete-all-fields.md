...
- [ ] 7 个标准字段与自定义字段添加后均能通过删除按钮移除并写回文件
- [ ] 标准字段删除有确认提示（文案带「由 NexNote 维护」等语义差异），自定义字段直接删除
- [ ] 删除后字段目录对应项恢复可添加（不再显示「已添加」禁用态）
- [ ] 删除字段不影响其他字段与正文；YAML 序列化结果中该键消失
- [ ] 标准字段仍不可重命名；折叠/类型切换等既有交互不回归
- [ ] 单测覆盖：标准字段删除、删除后目录状态、删除后保存往返、删除 `updated` 后文件不再出现该键
