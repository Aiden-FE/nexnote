...
**仓库内副本（Vault Copy）**:
经导入进入知识库的 docx / xlsx / xmind 等二进制文件；导入时内容与外部原件一致，此后与原件脱钩，作为知识库内合规文件被直接原地读写，可随 Git 版本化、可直接交给外部软件使用。
_Avoid_: 附件、原文件、导入件、副本编辑

**二进制文档（Binary Document）**:
知识库内以二进制格式存储、由应用内二进制编辑器直接打开编辑的文档（docx / xlsx / xmind），区别于由块组成的页面（Page）。
_Avoid_: 附件、非文本文件、媒体

**应用内二进制编辑器（In-App Binary Editor）**:
在产品内直接打开并编辑二进制文档的编辑器，提供类似 WPS / 语雀的编辑预览体验；不经过块编辑或 Markdown 源码视图。
_Avoid_: 预览器、只读视图、转换器

**只读保留区（Preserved Read-Only Region）**:
二进制文档中当前应用内编辑器无法编辑但必须原样保留的内容（如 xlsx 宏、图表、透视表、xmind 高级主题样式）；在编辑器内只读标注呈现，保存时原字节不丢失。
_Avoid_: 不支持区域、降级区、丢失内容
